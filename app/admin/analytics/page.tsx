...
export default async function AnalyticsPage() {
        ...
        <Card>
          <CardContent className="p-6">
            <p className="text-red-600">
              ...
              {lentError?.message || membersError?.message || gearError?.message}
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const recentRentals = lentItems.filter(
    (item) => item.lent_date && new Date(item.lent_date) >= thirtyDaysAgo,
  );
  ...
  const gearUtilization = gear
    .map((gearItem) => {
      const rentalCount = lentItems.filter((rental) => rental.gear_id === gearItem.id).length;

      const activeRentalCount = activeRentals.filter(
        (rental) => rental.gear_id === gearItem.id,
      ).length;
  ...
  const categoryStats = gear.reduce(
    (acc, gearItem) => {
      ...
      acc[category].activeRentals += activeRentals.filter(
        (rental) => rental.gear_id === gearItem.id,
      ).length;
  ...
  const categoryPerformance = Object.entries(categoryStats)
    .map(([category, stats]) => ({
      ...
      ...stats,
      utilizationRate:
        stats.totalItems > 0 ? Math.round((stats.activeRentals / stats.totalItems) * 100) : 0,
      averageRentalsPerItem:
        stats.totalItems > 0 ? Math.round((stats.totalRentals / stats.totalItems) * 10) / 10 : 0,
    }))
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-600">{recentRentals.length} in last 30 days</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-600">+{recentMembers.length} new members</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">
              {categoryPerformance.length > 0
                ? Math.round(
                    categoryPerformance.reduce((acc, cat) => acc + cat.utilizationRate, 0) /
            ...
            {categoryPerformance.map((category, index) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 border-l-4 ${colors[index % colors.length]} bg-gray-50 rounded-r-lg`}
                >
      ...
      {stats.length > 0 && <MonthlyStatsChart monthlyStats={stats} />}
