...
export default function RentalChart({ rentals }: RentalChartProps) {
  ...
  const categoryBreakdown = currentMonthRentals.reduce(
    (acc, rental) => {
  ...
  const colors = ["#3b82f6", "#10b981", "#8b5cf6", "#f59e0b", "#ef4444", "#eab308"];
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-white border border-gray-300 rounded-lg shadow-lg p-3">
                          ...
                          <p className="text-blue-600">{`Total Rentals: ${payload[0]?.value}`}</p>
                          <p className="text-green-600">{`Returned: ${payload[1]?.value}`}</p>
                          <p className="text-orange-600">{`Active: ${payload[2]?.value}`}</p>
                        </div>
                      );
                    }
                    return null;
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-white border border-gray-300 rounded-lg shadow-lg p-3">
                          ...
                          <p className="text-blue-600">{`${data.count} rentals`}</p>
                ...
                <Legend
                  ...
                  height={36}
                  formatter={(value: string) => <span className="capitalize text-sm">{value}</span>}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
