...
export default function GalleryAdmin({ initialImages }: { initialImages: GalleryRow[] }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const groupId = (globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2,8)}`);
      const created: GalleryRow[] = [];
      for (let idx = 0; idx < files.length; idx++) {
        const f = files[idx];
        const processed = await compressImageFile(f, { maxWidth: 1600, maxHeight: 1200, quality: 0.75 });
        ...
        const res = await fetch("/api/admin/gallery", {
          ...
          body: JSON.stringify({ title: caption, image_url: url, group_id: groupId, sequence: idx })
        });
        if (!res.ok) {
          const t = await res.text();
          // attempt cleanup of uploaded file
          await fetch(`/api/admin/gallery/upload?url=${encodeURIComponent(url)}`, { method: "DELETE" }).catch(() => {});
  ...
  const del = async (row: GalleryRow) => {
    ...
    try {
      ...
      await fetch(`/api/admin/gallery/upload?url=${encodeURIComponent(row.image_url)}`, { method: "DELETE" }).catch(() => {});
      const res = await fetch(`/api/admin/gallery?id=${encodeURIComponent(row.id)}`, { method: "DELETE" });
      if (!res.ok) {
        const t = await res.text();
  ...
  const updateCaption = async (groupKey: string, newCaption: string) => {
    ...
    const payload = { group_id: realGroupId, ids: group.items.map((i) => i.id), caption: newCaption };
    const res = await fetch("/api/admin/gallery/reorder", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (!res.ok) {
      const t = await res.text();
      return alert(t || "Failed to update caption");
    }
    setImages((prev) => prev.map((i) => i.group_id === realGroupId ? { ...i, title: newCaption } : i));
  };
  ...
  const reorderInGroup = async (groupKey: string, nextIds: string[]) => {
    ...
    if (!group) return;
    const realGroupId = group.group_id;
    const res = await fetch("/api/admin/gallery/reorder", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ group_id: realGroupId, ids: nextIds }) });
    if (!res.ok) {
      const t = await res.text();
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end">
        <div className="flex-1 grid gap-2">
          ...
          <input id="title" name="title" className="border rounded px-3 py-2" placeholder="Sunset at Algonquin" value={title} onChange={(e)=>setTitle(e.target.value)} aria-required />
        </div>
        <div className="grid gap-2">
          <label htmlFor="gallery-file" className="text-sm font-medium">Images</label>
          <input id="gallery-file" name="file" type="file" accept="image/*" multiple required className="block" onChange={onFileChange} />
          <p className="text-xs text-gray-500">Select multiple images to group under the same caption. Images are auto-optimized.</p>
        </div>
  <button type="button" onClick={upload} disabled={busy || files.length === 0 || !title.trim()} className="bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      <FolderTile
        ...
        {expandedGroup && expandedGroup !== 'UNGROUPED' && (
          (() => {
              ...
              <div className="border rounded-lg bg-white shadow-sm">
                <div className="flex items-center justify-between gap-3 p-4 border-b">
                  <div className="flex items-center gap-2 min-w-0 w-full">
                    ...
                    />
                    <span className="text-xs text-gray-500 whitespace-nowrap">{g.items.length} photo{g.items.length===1?"":"s"}</span>
                  </div>
                </div>
...
function GroupReorderGrid({ group, onDelete, onReorder }: { group: Group; onDelete: (row: GalleryRow) => void; onReorder: (groupKey: string, ids: string[]) => void }) {
  const [ids, setIds] = useState(group.items.map((i) => i.id));
  useEffect(() => { setIds(group.items.map((i)=>i.id)); }, [group.items.map((i)=>i.id).join(',')]);
    ...
    <div className="p-4 space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {visibleIds.map((id) => {
          ...
          if (!img) return null;
          return (
            <div key={id} className="border rounded overflow-hidden bg-white" draggable onDragStart={(e)=>onDragStart(e, id)} onDragOver={onDragOver} onDrop={(e)=>onDrop(e, id)}>
              ...
              <div className="p-2 flex items-center justify-between gap-2">
                ...
                <button type="button" onClick={() => onDelete(img)} className="text-red-600 text-sm">Delete</button>
              </div>
            </div>
          );
        })}
      ...
      <div className="text-right">
        <button type="button" onClick={saveOrder} className="bg-gray-900 text-white px-3 py-2 rounded">Save order</button>
      </div>
    </div>
  );
}
...
function FolderTile({ title, count, thumbs, icon, highlight, onClick, onDropItem }:{
  title: string;
  count?: number;
  thumbs?: string[];
  ...
  return (
    <div onClick={onClick} onDragOver={onDragOver} onDrop={onDrop} className={`group relative cursor-pointer select-none rounded-lg border ${highlight ? 'border-dashed border-blue-400 bg-blue-50' : 'bg-white'} p-3 shadow-sm hover:shadow-md transition`}> 
      <div className="flex items-center gap-2">
        {icon ?? <Folder className="h-5 w-5 text-gray-600" />}
        <div className="truncate text-sm font-medium">{title}</div>
        {typeof count === 'number' && <span className="ml-auto text-xs text-gray-500">{count}</span>}
      </div>
...
async function moveImage(id: string, target_group_id: string | null) {
  const res = await fetch('/api/admin/gallery/move', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, target_group_id }) });
  if (!res.ok) {
    const t = await res.text();
