...
export function ShoppingCartSheet() {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      </SheetTrigger>        <SheetContent className="w-[350px] sm:w-[400px] lg:w-[540px] max-w-[90vw]">
        <SheetHeader>
          <SheetTitle>Rental Cart ({cartItems.length} items)</SheetTitle>
          <SheetDescription>
            Review your selected gear rentals
          </SheetDescription>
        </SheetHeader>
    ...
    <div className="mt-6 flex-1 overflow-y-auto">
      {cartItems.length === 0 ? (
        ...
        <div className="space-y-4" aria-label="Items in your cart">
          {cartItems.map((item) => (
            <Card key={item.id} className="p-4" role="article" aria-labelledby={`item-${item.id}-name`}>
              <div className="flex gap-3">
                ...
                <div className="flex flex-col gap-2 items-end justify-between min-w-0 flex-shrink-0">
                  <p className="font-semibold text-sm whitespace-nowrap" aria-label={`Price: $${item.price}`}>
                    ${item.price}
                  </p>
